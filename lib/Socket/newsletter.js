...
const makeNewsletterSocket = (config) => {
    ...
    return {
        ...
        newsletterFetchAllParticipating: async () => {
        	...
        	const childStr = WABinary_1.getBinaryNodeChild(result, 'result')?.content?.toString()
        	if (!childStr) return data

        	let child
        	try {
        	    child = JSON.parse(childStr)
        	} catch (e) {
        	    return data
        	}
...
const extractNewsletterMetadata = (node, isCreate) => {
    const resultStr = WABinary_1.getBinaryNodeChild(node, 'result')?.content?.toString()
    ...
    const pathKey = isCreate ? Types_1.XWAPaths.CREATE : Types_1.XWAPaths.NEWSLETTER
